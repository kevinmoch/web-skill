    ...
    <div className="space-y-6" id="overview-screen">
      ...
      <div className="bg-card border border-border rounded-xl p-5 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h2 className="text-xl font-bold text-foreground tracking-tight mt-1.5 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-card border border-border rounded-xl p-5 shadow-xs">
            <div className="flex items-center justify-between mb-4 select-none">
              <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider font-mono flex items-center">
                <Workflow className="w-3.5 h-3.5 mr-1.5 text-foreground" />
                {lang === 'zh' ? '当前活跃迭代周期' : 'ACTIVE EXECUTING SPRINT'}
              </h3>
              <span className="text-xs font-semibold text-foreground font-mono">{activeSprint.progress}% Complete</span>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3 pt-3 border-t border-border font-mono text-center select-none">
                ...
                <div key={suite.id} className="flex items-center justify-between p-3 rounded-lg bg-secondary/40 border border-border">
                  ...
                  <div className="text-right flex items-center space-x-3 shrink-0">
                    <div className="text-right">
                      ...
                      <div className="text-[12px] text-muted-foreground mt-0.5 font-mono">{suite.passRate}% pass</div>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card border border-border rounded-xl p-5 shadow-xs">
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider font-mono mb-4 flex items-center">
              <BugIcon className="w-3.5 h-3.5 mr-1.5 text-destructive opacity-85" />
            ...
            <div className="space-y-4 select-none">
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between text-xs font-medium text-foreground">
                  <div className="flex items-center space-x-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-destructive/80 inline-block shrink-0" />
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between text-xs font-medium text-foreground">
                  <div className="flex items-center space-x-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-warning/80 inline-block shrink-0" />
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between text-xs font-medium text-foreground">
                  <div className="flex items-center space-x-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-success/80 inline-block shrink-0" />
...
export const RequirementsScreen: React.FC = () => {
  ...
  return (
    <div className="space-y-6 text-xs" id="requirements-screen">
      {/* Page Header */}
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-center bg-card p-3 rounded-xl border border-border" id="requirements-filter-bar">
        ...
        <select
          ...
          className="w-full sm:w-44 text-xs bg-background text-foreground border border-input rounded-lg px-3 py-2 focus:outline-none focus:border-ring font-medium cursor-pointer"
        >
          <option value="All">{lang === 'zh' ? '⚡ 过滤全部优先级' : 'Filter Priority: All'}</option>
          <option value="High">{lang === 'zh' ? '🔴 高优先级' : 'High Priority'}</option>
          <option value="Medium">{lang === 'zh' ? '🟡 中优先级' : 'Medium Priority'}</option>
          <option value="Low">{lang === 'zh' ? '🟢 低优先级' : 'Low Priority'}</option>
        </select>
      </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-secondary/60 border-b border-border text-muted-foreground font-semibold uppercase tracking-wider">
                <th className="p-3.5 w-24 font-mono whitespace-nowrap">{lang === 'zh' ? '需求 ID' : 'Key ID'}</th>
                <th className="p-3.5 min-w-[240px]">{lang === 'zh' ? '概况信息与描述' : 'Summary & Scope'}</th>
                <th className="p-3.5 whitespace-nowrap">{lang === 'zh' ? '史诗模块' : 'Epic Story'}</th>
                <th className="p-3.5 whitespace-nowrap">{lang === 'zh' ? '优先级' : 'Priority'}</th>
                <th className="p-3.5 whitespace-nowrap">{lang === 'zh' ? '状态' : 'Status'}</th>
                <th className="p-3.5 whitespace-nowrap">{lang === 'zh' ? '经办人' : 'Assignee'}</th>
                <th className="p-3.5 whitespace-nowrap text-center">{lang === 'zh' ? '点数' : 'SP'}</th>
                <th className="p-3.5 whitespace-nowrap text-right">{lang === 'zh' ? '操作' : 'Actions'}</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={8} className="text-center py-10 text-xs text-muted-foreground italic">
                    {lang === 'zh' ? '暂无匹配的敏捷需求或故事记录' : 'No Scrum backlog issues matched search.'}
                  </td>
                </tr>
              ) : (
                filtered.map((req) => (
                  <tr key={req.id} className="border-b last:border-0 border-border hover:bg-accent/40 transition">
                    {/* KEY ID */}
                    <td className="p-3.5 font-mono font-bold text-foreground select-all whitespace-nowrap">{req.id}</td>
                    ...
                    <td className="p-3.5 whitespace-nowrap">
                      {(lang === 'en' && req.epic_en ? req.epic_en : req.epic) ? (
                        <span className="px-2 py-0.5 rounded-md bg-accent text-accent-foreground border border-border font-mono text-[12px] font-medium inline-flex items-center gap-1 whitespace-nowrap">
                    ...
                    <td className="p-3.5 whitespace-nowrap">
                      <span
                        className={`px-2 py-0.5 rounded-full text-[12px] font-mono font-medium border whitespace-nowrap inline-flex items-center gap-1 ${
                          req.priority === 'High'
                    ...
                    {/* ASSIGNEE */}
                    <td className="p-3.5 whitespace-nowrap text-foreground font-medium font-mono text-[12px]">
                      <span>{(lang === 'en' && req.assignee_en ? req.assignee_en : req.assignee) || 'Unassigned'}</span>
                    </td>
              ...
              <form onSubmit={handleSave} className="space-y-4 pt-1 text-foreground">
                ...
                <div className="flex justify-end space-x-2 pt-3 border-t border-border">
                  <button
                    ...
                    className="btn-ghost px-3.5 py-1.5 text-xs cursor-pointer"
                  >
                    {t.actions.cancel}
                  </button>
...
export const SprintsScreen: React.FC = () => {
  ...
  return (
    <div className="space-y-6 text-xs" id="sprints-board-view">
      {/* Page Header */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5" id="scrum-kanban-rack">
        {/* Column 1: TO DO */}
        <div className="bg-slate-500/[0.08] dark:bg-slate-500/[0.16] rounded-xl p-3.5 flex flex-col h-[70vh] border border-slate-300/80 dark:border-slate-700/60 shadow-xs">
                ...
                >
                  ...
                  <div className="flex items-center justify-between border-t border-border pt-2 mt-2 select-none">
                    ...
                    <button
                      ...
                      className="px-2 py-0.5 bg-secondary border border-border text-foreground rounded text-[12px] font-medium hover:bg-accent transition cursor-pointer"
                    >
        ...
        <div className="bg-sky-500/[0.08] dark:bg-sky-500/[0.16] rounded-xl p-3.5 flex flex-col h-[70vh] border border-sky-300/80 dark:border-sky-700/60 shadow-xs">
          <div className="flex items-center justify-between mb-3 px-1 select-none">
            <div className="flex items-center space-x-1.5">
                  ...
                  <div className="flex items-center justify-between border-t border-border pt-2 mt-2 select-none">
                    ...
                    <div className="flex space-x-1">
                      <button
                        ...
                        className="px-1.5 py-0.5 bg-secondary hover:bg-accent text-muted-foreground hover:text-foreground rounded text-[12px] transition cursor-pointer"
                      >
                        ← Todo
                      </button>
                      <button
                        onClick={() => handleMoveStatus(tc.id, 'Done')}
                        className="px-1.5 py-0.5 bg-success-soft border border-success/20 text-success rounded text-[12px] font-medium hover:bg-success hover:text-white transition cursor-pointer"
                      >
                        {lang === 'zh' ? '提交DONE' : 'Done ✔'}
                      </button>
                    </div>
        ...
        {/* Column 3: DONE */}
        <div className="bg-emerald-500/[0.08] dark:bg-emerald-500/[0.16] rounded-xl p-3.5 flex flex-col h-[70vh] border border-emerald-300/80 dark:border-emerald-700/60 shadow-xs">
          <div className="flex items-center justify-between mb-3 px-1 select-none">
            <div className="flex items-center space-x-1.5">
          ...
          <div
            className="flex-1 overflow-y-auto space-y-2.5 pr-1 text-card-foreground"
            onDragOver={(e) => e.preventDefault()}
            onDrop={(e) => {
              const ticketId = e.dataTransfer.getData('text/plain');
              ...
              doneTickets.map((tc) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between border-t border-border pt-2 mt-2 select-none">
                    ...
                    <button
                      onClick={() => handleMoveStatus(tc.id, 'In Progress')}
                      className="px-1.5 py-0.5 bg-secondary hover:bg-accent border border-border text-muted-foreground hover:text-foreground rounded text-[12px] cursor-pointer"
                    >
                      ↺ {lang === 'zh' ? '重新开启' : 'Reopen'}
                    </button>
                  </div>
...
export const BugsScreen: React.FC = () => {
  ...
  return (
    <div className="space-y-6 text-xs" id="bugs-screen">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-foreground tracking-tight flex items-center gap-2">
      ...
      <div className="rounded-xl border border-border overflow-hidden bg-card shadow-xs" id="bugs-database">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-secondary/60 border-b border-border text-muted-foreground font-semibold uppercase tracking-wider">
                ...
                <th className="p-3.5 whitespace-nowrap">{lang === 'zh' ? '经办接收人' : 'Assignee'}</th>
                <th className="p-3.5 whitespace-nowrap">{lang === 'zh' ? '创建日期' : 'Created At'}</th>
                <th className="p-3.5 whitespace-nowrap text-right">{lang === 'zh' ? '操作' : 'Actions'}</th>
              </tr>
            </thead>
            <tbody>
              {bugs.length === 0 ? (
                ...
                bugs.map((bug) => (
                  <tr key={bug.id} className="border-b last:border-0 border-border hover:bg-accent/40 transition">
                    ...
                    {/* Subsystem Module */}
                    <td className="p-3.5 whitespace-nowrap text-foreground">
                      <span className="inline-flex items-center space-x-1 font-mono font-medium">
                        <Code className="w-3.5 h-3.5 text-muted-foreground" />
                        <span>{lang === 'en' && bug.module_en ? bug.module_en : bug.module}</span>
                      </span>
                    </td>
          ...
          <div id="bug-crud-modal" className="absolute inset-0 z-[1100] flex items-center justify-center p-4">
            ...
            <div className="bg-popover text-popover-foreground rounded-xl border border-border shadow-pop w-full max-w-md p-6 max-h-[92vh] overflow-y-auto relative z-10 flex flex-col font-sans">
              ...
              <form onSubmit={handleSave} className="space-y-4 pt-3 text-foreground">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium text-muted-foreground mb-1">{t.bugs.severity}</label>
                    <select
                      value={formSeverity}
                  ...
                  <div>
                    <label className="block text-xs font-medium text-muted-foreground mb-1">{t.bugs.status}</label>
                    <select
                      value={formStatus}
                      onChange={(e) => setFormStatus(e.target.value as any)}
                      className="w-full text-xs bg-background px-3 py-2 rounded-lg border border-input text-foreground focus:outline-none focus:border-ring font-medium cursor-pointer"
                ...
                {/* Module selection */}
                <div>
                  <label className="block text-xs font-medium text-muted-foreground mb-1">{t.bugs.module}</label>
                  <input
                    type="text"
                    required
                    value={formModule}
...
export const TestsScreen: React.FC = () => {
  ...
  return (
    <div className="space-y-6 text-xs" id="tests-screen">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-foreground tracking-tight flex items-center gap-2">
          ...
          <div key={suite.id} className="p-5 bg-card border border-border rounded-xl flex flex-col justify-between shadow-xs hover:border-foreground/30 transition text-card-foreground">
            <div>
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`px-2 py-0.5 rounded-sm text-[12px] font-mono font-medium uppercase border ${suite.status === 'Passed' ? 'bg-success-soft text-success border-success/20' : 'bg-destructive-soft text-destructive border-destructive/20'}`}
                >
                  {suite.status}
                </span>
              </div>
            ...
            <div className="border-t border-border pt-4 mt-4 space-y-3.5">
              <div className="grid grid-cols-2 gap-2 text-xs font-medium">
                <div>
                  <span className="text-muted-foreground block">{t.tests.coverage}</span>
                  <span className="text-sm font-bold font-mono text-foreground mt-1 block">{suite.coverage}%</span>
                </div>
                <div>
                  <span className="text-muted-foreground block">{t.tests.passRate}</span>
                  <span className="text-sm font-bold font-mono text-success mt-1 block">{suite.passRate}%</span>
                </div>
              </div>
          ...
          <div id="test-crud-modal" className="absolute inset-0 z-[1100] flex items-center justify-center p-4">
            ...
            <div className="bg-popover text-popover-foreground rounded-xl border border-border shadow-pop w-full max-w-md p-6 max-h-[92vh] overflow-y-auto relative z-10 flex flex-col font-sans">
              ...
              <form onSubmit={handleSave} className="space-y-4 pt-3 text-foreground">
                {/* Suite Name */}
                <div>
                  <label className="block text-xs font-medium text-muted-foreground mb-1">{t.tests.suiteName}</label>
                  <input
                    type="text"
                    required
                    value={formSuite}
    ...
    <div className="space-y-6 text-xs" id="metrics-reports-screen">
      ...
      <div className="flex overflow-x-auto select-none gap-2 mt-4 pb-1" id="metrics-tab-layout">
        <button
          ...
          className={`px-4 py-2 text-xs font-medium transition flex items-center shrink-0 space-x-1.5 cursor-pointer rounded-lg border ${
            activeTab === 'burndown'
              ? 'bg-accent border-border text-accent-foreground font-semibold shadow-xs'
              : 'bg-card text-muted-foreground hover:text-foreground border-border hover:bg-accent/50'
          }`}
        >
              ...
              <div className="space-y-5" id="report-burndown">
                ...
                <div className="h-64 relative font-sans">
                  <svg viewBox="0 0 500 180" className="w-full h-full text-muted-foreground select-none">
                    ...
                    <circle cx="130" cy="25" r="3" fill="currentColor" className="text-foreground/75" />
                    <circle cx="210" cy="50" r="3" fill="currentColor" className="text-foreground/75" />
                    <circle cx="290" cy="90" r="3" fill="currentColor" className="text-foreground/75" />
                    <circle cx="370" cy="120" r="3" fill="currentColor" className="text-foreground/75" />
                    <circle cx="450" cy="142" r="3.5" fill="var(--success)" opacity="0.85" />

                    {/* X labels */}
                    <text x="50" y="173" fontSize="8" textAnchor="middle" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '第1天' : 'Day 1'}
                    </text>
                    <text x="130" y="173" fontSize="8" textAnchor="middle" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '第3天' : 'Day 3'}
                    </text>
                    <text x="210" y="173" fontSize="8" textAnchor="middle" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '第5天' : 'Day 5'}
                    </text>
                    <text x="290" y="173" fontSize="8" textAnchor="middle" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '第7天' : 'Day 7'}
                    </text>
                    <text x="370" y="173" fontSize="8" textAnchor="middle" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '第9天' : 'Day 9'}
                    </text>
                    <text x="450" y="173" fontSize="8" textAnchor="middle" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '第10天 (结束)' : 'Day 10 (End)'}
                    </text>

                    {/* Y labels */}
                    <text x="35" y="28" fontSize="8" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '40 故事点' : '40 SP'}
                    </text>
                    <text x="35" y="53" fontSize="8" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '30 故事点' : '30 SP'}
                    </text>
                    <text x="35" y="93" fontSize="8" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '20 故事点' : '20 SP'}
                    </text>
                    <text x="35" y="133" fontSize="8" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '10 故事点' : '10 SP'}
                    </text>
                    <text x="35" y="163" fontSize="8" fontWeight="bold" fill="currentColor">
                      0
                    </text>
                  </svg>
                </div>

                <div className="p-3 bg-secondary/40 rounded-lg border border-border text-xs text-foreground flex items-start space-x-2 leading-relaxed">
              ...
              <div className="space-y-4" id="report-velocity">
                <div className="flex justify-between items-center bg-secondary/40 p-4 rounded-lg border border-border select-none">
                  ...
                  <div className="flex space-x-4 text-xs font-mono">
                    <span className="flex items-center">
                      <span className="w-2.5 h-2.5 bg-foreground rounded mr-1.5" />
                      {lang === 'zh' ? '预估点数' : 'Committed'}
                    </span>
                    <span className="flex items-center">
                      <span className="w-2.5 h-2.5 bg-success rounded mr-1.5" />
                ...
                <div className="h-64 relative font-sans">
                  <svg viewBox="0 0 500 180" className="w-full h-full text-muted-foreground select-none">
                    ...
                    {/* X Text Labels */}
                    <text x="110" y="173" fontSize="8" textAnchor="middle" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '迭代 1' : 'Sprint 1'}
                    </text>
                    <text x="200" y="173" fontSize="8" textAnchor="middle" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '迭代 2' : 'Sprint 2'}
                    </text>
                    <text x="290" y="173" fontSize="8" textAnchor="middle" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '迭代 3' : 'Sprint 3'}
                    </text>
                    <text x="380" y="173" fontSize="8" textAnchor="middle" fontWeight="bold" fill="currentColor">
                    ...
                    <text x="35" y="63" fontSize="8" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '30 故事点' : '30 SP'}
                    </text>
                    <text x="35" y="103" fontSize="8" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '15 故事点' : '15 SP'}
                    </text>
                    <text x="35" y="143" fontSize="8" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '5 故事点' : '5 SP'}
                    </text>
                    <text x="35" y="163" fontSize="8" fontWeight="bold" fill="currentColor">
                      0
              ...
              <div className="space-y-4" id="report-cfd">
                <div className="flex justify-between items-center bg-secondary/40 p-4 rounded-lg border border-border select-none">
                  ...
                  <div className="flex space-x-3 text-xs font-mono text-foreground">
                    <span className="flex items-center">
                      <span className="w-2.5 h-2.5 bg-success rounded-full mr-1.5 opacity-80" />
                      {lang === 'zh' ? '已收尾已关单 (DONE)' : 'DONE'}
                    </span>
                    <span className="flex items-center">
                      <span className="w-2.5 h-2.5 bg-info rounded-full mr-1.5 opacity-80" />
                      {lang === 'zh' ? '正在进行中 (IN PROGRESS)' : 'IN PROGRESS'}
                    </span>
                    <span className="flex items-center">
                      <span className="w-2.5 h-2.5 bg-muted-foreground rounded-full mr-1.5 opacity-80" />
                      {lang === 'zh' ? '待开发积压 (TODO)' : 'TODO'}
                    </span>
                  </div>
                </div>

                {/* Cumulative flow stream chart */}
                <div className="h-64 relative font-sans">
                  <svg viewBox="0 0 500 180" className="w-full h-full text-muted-foreground select-none">
                    {/* Horizontal reference lanes */}
                    <line x1="50" y1="20" x2="450" y2="20" stroke="currentColor" strokeOpacity="0.08" strokeDasharray="3" />
                    ...
                    <text x="116" y="173" fontSize="8" textAnchor="middle" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '第 2 周' : 'W2'}
                    </text>
                    <text x="182" y="173" fontSize="8" textAnchor="middle" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '第 3 周' : 'W3'}
                    </text>
                    <text x="248" y="173" fontSize="8" textAnchor="middle" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '第 4 周' : 'W4'}
                    </text>
                    <text x="314" y="173" fontSize="8" textAnchor="middle" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '第 5 周' : 'W5'}
                    </text>
                    <text x="380" y="173" fontSize="8" textAnchor="middle" fontWeight="bold" fill="currentColor">
                      {lang === 'zh' ? '第 6 周' : 'W6'}
