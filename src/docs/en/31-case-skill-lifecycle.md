...
One more useful thing: if you uploaded a template file in this session (say an Excel quotation template), saving as a skill can pack it into the skill package as-is, so using the skill later doesn't need a re-upload. For the full how-to, see [Case: Producing Files from Your Own Template](#/docs/case-template).

> **Note**: During this task it may have asked you "Run parts of this task in parallel?" — that's delegation: it hands the task to several lanes working at the same time. That confirmation card states it plainly: what they do is not recorded in this conversation, so this conversation can no longer be saved as a new skill. If you want to save a skill afterwards, choose **Deny** when it asks — declining means the assistant does the work itself in this conversation and won't ask again. A round that already delegated can't be saved; the fix is to not let it delegate, redo the key steps, then save. For the reason, see [Seeing What the Assistant Is Doing](#/docs/transparency).
