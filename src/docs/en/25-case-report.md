...
After sending, expand the **Run flow** — it's the thing most worth watching in this chapter. You can see the assistant first read this skill's instructions through, then run the skill by those instructions: the skill's script goes to the various screens of the system you're in and fetches the four kinds of data — sprints, requirements, defects, tests — one by one, then aggregates them into the report. Every number in the report comes from a real fetch in this step, not something it made up.

![With the run flow expanded, it first reads the skill instructions, then runs the skill to fetch the data from each screen](/docs-assets/case-report/en/S-case-report-02-run-steps.png)

Behind one sentence, the assistant fetched the four kinds of data one by one following the skill, then aggregated. Sample run output — your actual output will differ.

Open the skill-running step itself to see exactly what it brought back:

![The skill-running step expanded, showing the data it brought back](/docs-assets/case-report/en/S-case-report-03-tool-detail.png)
...
![The artifacts section of a run's Inspector page, each file with preview and download](/docs-assets/case-report/en/S-case-report-05-artifact-download.png)
