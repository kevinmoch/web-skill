...
- Know what actually goes out when you send a Word file, an Excel file, a PowerPoint file, a pdf, or an image
- Know when an image met while reading a document may, with your consent, be sent to another model
...
| Doc text (docx, xlsx, pptx) | Read into text in your own browser before sending — what goes out is only the body text read out; images and layout in the document are not included — the original file is not sent in whole |
...
## Images met while reading documents may go to another model

The model you picked can't read images, and while reading a document it runs into some — the words in a scan, a chart, or an illustration genuinely can't be read without seeing them (having it read documents from your download folder, extension version only, works the same way). At that point it can send those few images to **the first connection in your model list that can read images**, asking that connection to describe them before carrying on. That connection is one you configured yourself in the model list, but it may belong to another model provider — meaning those few images leave the model service you currently picked. The rules of this channel:

- **The first time, an informational card pops up first**, titled "Send images to another model?", spelling out four things: why (the model you picked can't read images), to whom (the connection's name and address), what (which document produced these few images), and what happens if you decline (nothing is sent, and the turn carries on as if it hadn't seen the images).
- The card offers the checkboxes "Don't ask again in this conversation" and "Don't ask again for this endpoint". You can also leave them unchecked and simply decline — after a decline, this conversation won't ask again.
- **Images you upload by hand don't take this path.** When the current model can't read images, the upload entry blocks them outright ("the selected model does not accept images") — it doesn't accept them first and then pass them to someone else.
- **Page screenshots don't trigger this channel.** When the model can't read images, page reading simply doesn't capture images — there's no "ask a question about the page, and an extra model round-trip happens behind your back".

## Images placed into documents: from the page and from the web, asked separately

When generating a document or an export, if it wants to place an image into it, it asks you in separate cards by source: taking a picture from **the current page** into the document is asked once; **downloading** an image from **the internet** into the document is asked separately — and the card states plainly: downloading tells that server your network address and when you asked for it. Each kind has its own "don't ask again in this conversation" checkbox, and checking one doesn't clear the other. Only the extension version can actually download a linked image into an export; in the web version's exports, this kind of image remains a link.
