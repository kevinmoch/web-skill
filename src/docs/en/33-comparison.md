...
| PPTX attachments (read through the same path as Word and Excel) | Yes | Yes | [Attachments, Images, Voice, and Camera](#/docs/attachments) |
| Parallel delegation (it hands the task to several lanes working at once, asking you first; a conversation that used delegation can't be saved as a new skill) | Yes | Yes | [Seeing What the Assistant Is Doing](#/docs/transparency) |
| Vision delegation (when the model can't read images, images met while reading a document are, with your consent, sent to a connection that can) | Yes | Yes | [Privacy and Where Your Data Goes](#/docs/privacy) |
...
| Hover, drag, horizontal scroll | Yes | Yes | [Letting the Assistant Act on the Page](#/docs/page-actions) |
| Reading WPS online documents | No | Yes (read-only: reads content and takes screenshots, never writes back to the document) | [Letting the Assistant Read the Page](#/docs/page-perception) |
| DWG drawing viewing, measuring, and comparing | Yes | Yes (plus one more channel: reading drawings straight from the browser's download list) | [DWG Drawings: View, Measure, and Compare](#/docs/dwg-viewer) |
...
| Saving as an editable PPTX / DOCX | Yes | Yes (the export button on the window's toolbar; content matches the screen, the layout may differ) | [Artifacts: Dashboards, Documents, Slides, and Printing](#/docs/artifacts) |
| Producing files from templates (fill data into your Excel/Word template to produce a new file) | Yes | Yes (and can also use an online WPS document directly as a template) | [Case: Producing Files from Your Own Template](#/docs/case-template) |
| Linked images into exports | Limited (the export keeps only the link) | Yes (with your authorization, downloads the linked image into the export) | [Artifacts: Dashboards, Documents, Slides, and Printing](#/docs/artifacts) |
