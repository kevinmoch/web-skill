...
**You send a message and nothing happens at all.** The standing guarantee is: every send either shows a run state (the **Run flow** card starts rolling) or produces a visible failure notice — never neither. If truly nothing appeared, refresh the page and send again — the session and what's already generated won't be lost. If it keeps happening, expand that run's **Run flow**, copy the Run ID, and check where it stopped in the Console's [Run History](#/docs/console-runs); if you can't find it there, report the problem as in "When it's not covered here" at the end of this chapter.

**You're told "No model configured".** There's no usable model yet. Click **Configure model** to add one: for how to fill in the address and key see [Connections](#/docs/console-connections); for how to choose see [Choosing a Model](#/docs/models).
