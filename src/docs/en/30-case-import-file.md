...
- A requirement row with an attachment has a .docx link in it — click to download; they're ready-made practice files, and this chapter uses a few of them.

## Step 1: Hand the document to the assistant

1. Drag the docx files you just downloaded into the input field together, or click **Attach a file** below the input field to pick them all at once.
2. The chip's type label reads "Doc text", and you can remove it any time before sending.

![Dragging several docx files into the input field, the attachment chips' type label showing "Doc text"](/docs-assets/case-import-file/en/S-case-import-01-docx-chip.png)
...
- **The document is long, and it acts like it didn't finish it.** A single document is read in at most 32K characters at a time; the excess is truncated and marked as such. It keeps the original, so there's no need to re-upload it in segments — just tell it to "keep reading the rest" and it continues page by page, chunk by chunk. You can also state up front which part you want — "only enter the acceptance criteria in chapter 3", for example.
...
- **You sent a very large PDF.** When it exceeds the reading limit, it automatically switches to sending the text read out of the file instead of the original, and says so in the message (layout, charts, and scanned pages are not included) — for the details, see [Attachments, Images, Voice, and Camera](#/docs/attachments).
