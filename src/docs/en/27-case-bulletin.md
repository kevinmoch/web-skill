...
One thing to know about how it works today: when the assistant writes a document on the spot, **what comes out by default is a plain Word document** — title, body, charts, and tables, but no red letterhead, document number, or signature block. If you want a red-letterhead official document, your sentence has to say "red letterhead", "official document", or "bulletin" out loud. The sentence above carries "bulletin", so it takes the red-letterhead path; the same goes for the revised version in step 4 — it remembers you asked for a bulletin.
...
1. In the document window's top-right corner, click **Print** — use this button, not the browser menu's print. The document still carries an old line of text telling you to use the browser's print command — ignore it; the **Print** button on the window's toolbar is the one that counts.
...
On the "made on the spot" version, the window's toolbar has an **Export DOCX** button — present in both the web and extension versions — which saves the document as an editable Word file you can take away. The content matches what you see on screen; the layout may not be exactly the same as on screen.
...
The flow is the same in both — even the **Export DOCX** button that saves an editable Word copy sits on the window's toolbar in both the web and extension versions (see above). The difference is the data scope: the web version only pulls data from the current system; on the extension version, a "made on the spot" skill can also read your other open tabs before aggregating (see [Working Across Tabs](#/docs/tabs)).
