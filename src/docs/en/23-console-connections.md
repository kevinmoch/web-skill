...
- Drag the handle on the left of a row to reorder it, or focus the handle and use the up and down arrow keys (the hint reads: "Move {label} — drag, or press the up and down arrow keys"). With two or more models in the list, the top of the page spells out what this order governs: "This order is your preference order. When the model in use cannot read images, the topmost image-capable model here is the one asked to describe them." — what the capability badges mean: [Choosing a Model](#/docs/models)
...
> **Extension only**: When the extension updates, the built-in models shipped with it are replaced wholesale with the new configuration and put back into the default order; models you added yourself are untouched and stay behind them. Day to day, your choice of default model and your drag ordering always hold; only when the extension repackages its model configuration, or you reinstall the extension, are these changes reset.
...
- Turn on the **Enable WebMCP** master switch, and the tools the current web page reports are listed below; when the browser supports WebMCP but it isn't on yet, the page states "Available in this browser, but not enabled. Turn it on to expose page tools to the model."
- Then set each tool in the list individually to Enabled / On demand / Disabled; when the master switch is on but the page hasn't reported a single tool, the page states "WebMCP is enabled but exposes no tools."
...
- When the page is made of several regions (the page body and each embedded page region each count as a source), the list groups by source: each group gets its own master switch, and the group name carries one of three states — Not supported / Supported, not enabled / Enabled; turning one source off doesn't implicate the others

![The WebMCP Tools page: one master switch plus the tool list, each tool toggled individually](/docs-assets/console-connections/en/S-console-conn-03-webmcp.png)

The master switch, the per-tool toggles, and the not-enabled and no-tools notices all live on this page.
