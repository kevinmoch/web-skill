...
![The Library showing all 14 of the Demo's built-in skills on one page, with source, contents, and last used](/docs-assets/console-skills/en/S-console-skills-01-library.png)
...
- The toolbar also has **Upload files**: it sends files from your machine straight into the currently selected folder. Any type works, and you can pick several at once; a name clash first pops a "File already exists" prompt — the current contents are archived as a new version first, and only clicking **Replace** overwrites; anything over the size limit (10 MB) is refused outright, with "Nothing was written" stated — no half a file left behind. When your system hasn't wired this capability, the entry doesn't show
- A binary file (an Excel template, say) doesn't open into the text editor — it gets a read-only info panel instead: the file name, the size, and a note that reads "Binary file. Not shown as text, and not editable here — saving it through a text editor would change the bytes." The panel has no save button, so there's no way to save mojibake back over the original file
