...
![Speaking into voice input, in the recording state](/docs-assets/case-fill-form/en/S-case-form-01.png)
...
![The assistant has jumped to the Requirements page and opened the Create Requirement form](/docs-assets/case-fill-form/en/S-case-form-02.png)

![The requirement form's fields filled in automatically, the cursor still on the form](/docs-assets/case-fill-form/en/S-case-form-03.png)
...
![M-28 One sentence becomes a record: spoken, understood, jumping to the right page, filling field by field, [STOP] the submit confirmation card, written successfully after Allow](/docs-assets/case-fill-form/en/M-28.svg)
...
![The submit confirmation card standing in front of the filled form](/docs-assets/case-fill-form/en/S-case-form-04.png)
...
- If everything looks right, click **Allow**: the record really is written into the system and appears in the list right away. The suggested value you just confirmed lands in it along the way — that's how the assignee column in the next image comes to be filled.

![After a successful submission, the new entry in the requirement list](/docs-assets/case-fill-form/en/S-case-form-05.png)

The same run as the images above: once allowed, the write is immediately visible. Sample run output — your actual output will differ.
...
![After a denied submission, the assistant explains the record wasn't written and continues with the rest](/docs-assets/case-fill-form/en/S-case-form-06.png)

Still the same run: denying blocks only this one record — it explains the record wasn't written, then carries on with the rest. Sample run output — your actual output will differ.
...
2. **You didn't say, but it remembers your habits.** The assignee and story points, for example. For these fields it **doesn't decide for you on its own** — it first places a suggested value, with a reason attached — "you've assigned it to this person the last few times". A suggested value is laid out for you to confirm: you click **Use** to take it, or nod in the plan it presents or at the submit confirmation, and only then does it fill the value into the form. So looking ahead in the same run, the assignee column is filled — that landed after you confirmed the suggestion, not on its own initiative. For the mechanics, see [Six Interaction Cards](#/docs/interactions).

![Field close-up: the suggested value and its reason visible at the same time](/docs-assets/case-fill-form/en/S-case-form-07.png)
