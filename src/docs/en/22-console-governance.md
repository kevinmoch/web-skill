...
2. Check the ID, source, risk, and creation time; high-risk ones list the reasons one by one (needing network access, writing outside the directory, and so on). Below is the complete content of every file in the skill — what's about to be published is read word by word here. When the package carries a binary file (an Excel template, say), it isn't shown as text — only its name and size are listed, with a reminder to confirm whether that file should travel with the skill package.
