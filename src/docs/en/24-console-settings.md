...
- In the **Agent capabilities** section, toggle one by one: **Task list**, **Generative UI**, **Skill generation** (the entry that turns a conversation into a skill — off at the factory, on by default in the Demo; the effect is in [Skills: Giving the Assistant Expertise](#/docs/skills-usage)), **Delegate sub-tasks** (it can hand sub-tasks to several runs at once, and before the first delegation a confirmation card pops up asking you to pick the maximum number of runs — see [Skills: Giving the Assistant Expertise](#/docs/skills-usage)). When the current model doesn't support tools, all four are greyed out with the reason stated on the page.
...
- The remaining items: **Failure quarantine threshold** — a skill that fails this many times in a row is quarantined automatically (5 by default; one success resets the count); whether answers stream out word by word (**Streaming responses**) and carry **Render result blocks** at the end; **Temperature (optional)** left empty uses the model vendor's default. The **Lifecycle hooks** section is up to your system: when the system has registered no hooks, the section shows the empty state "No lifecycle hooks are registered."; when the system hasn't wired this entry at all, the whole section doesn't appear.
- Three size limits, all filled in KB. **Tool result limit** governs how much one tool call may return; **Document limit** governs the documents one request may carry — attaching a file itself no longer has a hard size gate: the file is written straight into this device's storage, and when it won't fit, a "Not enough space on this device" notice pops up; a PDF over the document limit is sent as the text read out of the file instead, and only one with no extractable text is refused; **Extracted text limit** governs the text read out of files — over the limit likewise means refusal, not truncation, and it defaults to 5000 KB, so a thousand-page PDF that used to be refused now enters the conversation normally. Each of these notices states the actual size, the limit, and where to change it.
- **Context window (tokens, optional)** is filled in per model, and the entry is on the **LLM** page of [Connections](#/docs/console-connections): you fill it while editing a model. Once filled, the conversation shows above the input box the input tokens the latest send actually consumed and their share of the window; left empty, only the absolute token count shows, with no percentage. Making sense of this reading: [Seeing What the Assistant Is Doing](#/docs/transparency).
...
![The change preview before importing: how many added and overwritten, and the origin site](/docs-assets/console-settings/en/S-privacy-04-import-diff.png)
...
![The notice when a profile containing credentials is rejected](/docs-assets/console-settings/en/S-privacy-05-credential-rejected.png)
